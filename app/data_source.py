...
def fetch_ohlc_kraken(
    ...
    df = df[df["timeOpen"].dt.date <= end_date].reset_index(drop=True)  # only closed candles
...
def build_exact_training_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Timestamp]:
    ...
    tenkan = (df["high"].rolling(tenkan_n, min_periods=tenkan_n).max() + df["low"].rolling(tenkan_n, min_periods=tenkan_n).min()) / 2.0
    kijun = (df["high"].rolling(kijun_n, min_periods=kijun_n).max() + df["low"].rolling(kijun_n, min_periods=kijun_n).min()) / 2.0
    ...
    # Safety: ensure prices are positive
    row = df.iloc[-1]
